...
class VCOCO(torch.utils.data.Dataset):
    ...
    def __init__(self, img_set, img_folder, anno_file, transforms, num_queries, args):
        ...
        # change SS
        self.all_hois = set([i for i in range(len(self.text_label_ids))])
        self.unseen_index = []
        self.seen_hois = list(self.all_hois - set(self.unseen_index))
        self.seen_hoi2obj = torch.Tensor([self.text_label_ids[ind][1] for ind in self.seen_hois]).long()    
        self.seen_hoi2verb = torch.Tensor([self.text_label_ids[ind][0] for ind in self.seen_hois]).long()
        assert self.seen_hoi2obj.shape == self.seen_hoi2verb.shape

        self.num_seen_objs = torch.unique(self.seen_hoi2obj).shape
        self.obj_hoi_co_occurrence = torch.zeros(81, len(self.seen_hois))
        # must consider 80 objects of COCO + no object-involvement case

        for obj_ind in torch.unique(self.seen_hoi2obj):
            self.obj_hoi_co_occurrence[obj_ind] = self.seen_hoi2obj.eq(obj_ind).float()

    def __len__(self):
        # print(f'Training images: {len(self.annotations)}')
    ...
    def load_correct_mat(self, path):
        ...
        # print(len(self.correct_mat)) # 29
        # print(self.correct_mat[0]) # list of 80
...
def build(image_set, args):
    ...
    # change SS: folder names adjusted
    PATHS = {
        'train': (root / 'coco2014' / 'train2014', root / 'annotations' / 'trainval_vcoco.json'),
        'val': (root / 'coco2014' / 'val2014', root / 'annotations' / 'test_vcoco.json')
    }
