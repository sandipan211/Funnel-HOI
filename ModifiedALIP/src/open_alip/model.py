...
class VisualTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        # NOTE: verified that this function is same as for CLIP - so adding the contextual tokens line as in ModifiedCLIP
        ...
        # print(f'contextual features: {contextual.shape}')
        return x, contextual
...
class ALIP(nn.Module):
    def __init__(
            self,
            embed_dim: int,
            vision_cfg: ALIPVisionCfg,
            text_cfg: ALIPTextCfg,
            quick_gelu: bool = False,
    ):
        super().__init__()
        if isinstance(vision_cfg, dict):
            vision_cfg = ALIPVisionCfg(**vision_cfg)
        if isinstance(text_cfg, dict):
            text_cfg = ALIPTextCfg(**text_cfg)
    ...
    def forward(self, image, text):
        self.set_grad_checkpointing(enable=True)
        if image is None:
        ...
        text_features_raw = self.encode_text(text[:, :77])
        text_features_raw = F.normalize(text_features_raw, dim=-1)
        
        text_features_caption = self.encode_text(text[:, 77:])
        text_features_caption = F.normalize(text_features_caption, dim=-1)

        return image_features, text_features_raw, text_features_caption, self.logit_scale.exp()
