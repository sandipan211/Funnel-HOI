...
# import losses
from .losses import SIMCLRLoss, SLIPLoss, CLIPLoss
...
class VisionTransformerPatch(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.vit.blocks(x)      # (B, N_patches+1, D)

        # modifying the part below similar to ModifiedCLIP at https://github.com/Artanic30/HOICLIP/blob/ee4db062097410abdd20fa96d40d26aaca1f19da/ModifiedCLIP/model.py
        # Step 5: LayerNorm
        contextual = self.vit.norm(x)        # (B, N_patches+1, D)
        x = self.vit.norm(x[:, 0, :])

        # Step 6: Return CLS and patch embeddings 
        return x, contextual
...
class CLIP(nn.Module):
    ...
    def encode_image(self, image):
        x, contextual = self.visual(image)
        # print(x.shape) # (b, 768)
