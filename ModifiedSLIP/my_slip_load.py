...
import timm
from .models import SLIP, VisionTransformerPatch
...
def load(model_name, model_weight):
    ...
    # vision model only gives (bs, 768) - no patch level 3D embeddings - so writing a patch
    vmodel_patch = VisionTransformerPatch(vision_model)

    model_slip = SLIP(embed_dim=512, vision_width=768, vision_model=vmodel_patch, context_length=77, vocab_size=49408,
    ...
    # for name, param in model_slip.named_parameters():
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_name", default="ViT-B/16", help="Name of the vision backbone to use.")
