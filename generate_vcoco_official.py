...
from datasets.vcoco_text_label import vcoco_hoi_text_label, vcoco_obj_text_label_our, vcoco_verb_text_label
from datasets.datasets_gen.vcoco import build as build_dataset

from models.backbone import build_backbone
from models.models_hoiclip.gen import build_gen
...
class HOICLIP(nn.Module):
    def __init__(self, backbone, transformer, num_obj_classes, num_verb_classes, num_queries, args=None):
        ...
        self.transformer.hoi_cls = hoi_clip_label / hoi_clip_label.norm(dim=-1, keepdim=True)
    ...
    def generate_vcoco_text_label(self):
        ...
        obj_text_inputs = torch.cat([clip.tokenize(obj_text[1]) for obj_text in vcoco_obj_text_label_our])
...
class PostProcessHOI(nn.Module):
    ...
    def __init__(self, num_queries, subject_category_id, correct_mat, args):
        ...
        for index, hoi_pair in enumerate(self.vcoco_triplet_labels):
            self.hoi_obj_list.append(hoi_pair[1]) # the object appearing in ith HOI
            self.verb_hoi_dict[hoi_pair[0]].append(index) # the verb appearing in the ith HOI
    ...
    def forward(self, outputs, target_sizes):
        ...
        for index in range(len(hoi_scores)):
            hs, os, ol, sb, ob = hoi_scores[index], obj_scores[index], obj_labels[index], sub_boxes[index], obj_boxes[
                index]
            sl = torch.full_like(ol, self.subject_category_id) # sub category id by default set to 0 (for person class)
            l = torch.cat((sl, ol))
            b = torch.cat((sb, ob))
            ...
            hois = [{'subject_id': subject_id, 'object_id': object_id, 'category_id': category_id, 'score': score} for subject_id, object_id, category_id, score in zip(ids[:ids.shape[0] // 2].to('cpu').numpy(), ids[ids.shape[0] // 2:].to('cpu').numpy(), verb_labels, verb_scores)]
...
def main(args):
    ...
    gen = build_gen(args)
    model = HOICLIP(backbone, gen, len(valid_obj_ids) + 1, len(verb_classes),
...
def generate(model, post_processor, data_loader, device, verb_classes, missing_category_id):
    ...
    for samples, targets in metric_logger.log_every(data_loader, 10, header):
        ...
        outputs = model(samples, clip_img, targets)
